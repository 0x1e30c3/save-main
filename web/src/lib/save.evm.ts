...
import { EVM_CHAIN_HEX, EVM_RPC_URL, EVM_SAVE_ADDRESS } from '@/lib/config'
import type { SaveAccount, SaveService, YieldTarget } from '@/lib/types'
...
const iface = new Interface(SAVE_ABI)

type EthereumWindow = Window & { ethereum?: Eip1193Provider }

function toYieldTarget(index: bigint): YieldTarget {
  if (index === 1n) return 'blend'
  if (index === 2n) return 'soroswap'
  return 'defindex'
}

function fromYieldTarget(target: YieldTarget): number {
  if (target === 'blend') return 1
  if (target === 'soroswap') return 2
  return 0
}

function reader(): Contract {
  return new Contract(EVM_SAVE_ADDRESS, SAVE_ABI, new JsonRpcProvider(EVM_RPC_URL))
}
...
async function signerContract(): Promise<Contract> {
  ...
  return new Contract(EVM_SAVE_ADDRESS, SAVE_ABI, signer as ContractRunner)
}
...
export const saveEvm: SaveService = {
  async getAccount(user: string): Promise<SaveAccount> {
