...
// imported directly (not via 'save') because the bindings package's own generated
...
// Soroswap's real mainnet XLM/USDC pool (same asset pairing as Save's own testnet
...
// pool Save's own contract integrates with (Save is testnet-only); it exists purely to
...
// Shared by both the testnet pool Save actually integrates with and the mainnet reference
 ...
 * the reserve's current b_rate (the fixed-point index that converts a Save account's
 ...
 * BLEND_MAINNET_POOL_ID) - NOT the pool Save's contract actually integrates with. Exists
 ...
 * pool Save's own testnet integration uses - see getBlendMainnetReferenceApy for why a
 ...
 * applied to a real mainnet pool instead of Save's own contract. Unlike testnet - confirmed
