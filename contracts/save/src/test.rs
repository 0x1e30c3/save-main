...
use crate::{
    BlendPositions, BlendReserve, BlendReserveConfig, BlendReserveData, BlendRequest, Save,
    SaveClient, YieldTarget, BLEND_REQUEST_SUPPLY, BLEND_REQUEST_WITHDRAW,
...
// A single mock plays router, factory, and pair at once (Save calls all
...
impl MockSoroswapRouter {
    ...
    // Test-only: seeds the pool from a third-party LP so Save's own
...
// third party (never Save itself), roughly matching the live testnet
...
fn blend_withdraw_rounding_with_nonzero_remainder_zeroes_out_exactly() {
    ...
    // itself - that dust belongs to the pool, not to any Save user, and
...
fn soroswap_pay_swaps_half_and_mints_lp_shares() {
    ...
    // Save is the only depositor besides the pool seeder, so its shares
